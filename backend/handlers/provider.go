...
type AIProvider interface {
	// SendRequest forwards a unified payload to the provider.
	// apiKeyOverride, when non-empty, is used instead of the handler's default API key.
	SendRequest(ctx context.Context, payload []byte, apiKeyOverride string) ([]byte, error)
	ProviderName() string
}
